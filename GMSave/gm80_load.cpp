...
static char* make_delphi_str(const std::string& s)
{
    return make_delphi_str(s.c_str());
}
...
static void write_glob_u8(uint32_t off, uint8_t v)
{
    *(uint8_t*)(glob_base() + off) = v;
}
static void write_glob_u32(uint32_t off, uint32_t v)
{
    *(uint32_t*)(glob_base() + off) = v;
}
static void write_glob_bool(uint32_t off, bool v)
{
    write_glob_u8(off, v ? 1 : 0);
}
static void write_glob_i32(uint32_t off, int32_t v)
{
    *(int32_t*)(glob_base() + off) = v;
}
static void write_glob_f64(uint32_t off, double v)
{
    *(double*)(glob_base() + off) = v;
}
...
static int resolve_name_warn(const char* owner, const char* what, const std::string& name,
    ...
    const char* dirName, const fs::path& root)
{
    ...
    while (std::getline(ss, line))
    {
        ...
        std::string trimmed = (trimmedStart == std::string::npos)
            ? ""
            : line.substr(trimmedStart);
...
static void load_image_file_core(
    const fs::path& root, const char* fname, uint32_t globOff, uint32_t imgCls)
{
...
static void load_icon_file(const fs::path& root, const char* fname, uint32_t globOff)
{
    std::string data = read_file(root / "settings" / fname);
    if (data.empty()) return;
    uint8_t* base = (uint8_t*)g_load_base;
...
static void load_image_file_safe(
    const fs::path& root, const char* fname, uint32_t globOff, uint32_t imgCls)
{
    __try
    {
        load_image_file_core(root, fname, globOff, imgCls);
    }
...
static void load_icon_file_safe(const fs::path& root, const char* fname, uint32_t globOff)
{
    __try
    {
        load_icon_file(root, fname, globOff);
    }
    __except (EXCEPTION_EXECUTE_HANDLER)
    {
...
static void load_settings(const fs::path& root)
{
        ...
        [&](auto& k, auto& v)
    {
        if (k == "fullscreen") write_glob_bool(ADDR_SETTING_FULLSCREEN, v == "1");
        else if (k == "interpolate_pixels")
            write_glob_bool(ADDR_SETTING_INTERPOLATE, v == "1");
        else if (k == "dont_draw_border")
            write_glob_bool(ADDR_SETTING_DONT_DRAW_BORDER, v == "1");
            ...
            write_glob_bool(ADDR_SETTING_ALLOW_RESIZE, v == "1");
        else if (k == "window_on_top")
            write_glob_bool(ADDR_SETTING_WINDOW_ON_TOP, v == "1");
        else if (k == "set_resolution")
            write_glob_bool(ADDR_SETTING_SET_RESOLUTION, v == "1");
    ...
    // Version number globals from the root metadata (dword_5E943C..48)
    {
        ...
        for (auto& entry : fs::directory_iterator(root))
        {
            ...
            if (ext == ".gm80" ||
                entry.path().filename().string().find(".gm80") != std::string::npos)
            {
                metaFile = entry.path();
                break;
            }
    ...
    if (!constants.empty())
    {
        ...
        if (cnt > 0)
        {
            ...
            uint8_t* b = (uint8_t*)g_load_base;
            uint32_t fn = (uint32_t)b +
                0x6A38; // @DynArraySetLength (ADDR_DYN_ARRAY_SETLENGTH, defined later)
...
static void* load_sound(const std::string& name, const fs::path& sndDir)
{
    ...
    if (!src.empty())
        set_obj_str(snd, 16, utf8_to_ansi(src)); // file is UTF-8, GM stores AnsiString
    ...
    if (exists && !ext.empty())
    {
        ...
        if (file_exists(audioPath))
        {
            ...
            if (!audioData.empty())
            {
                void* ms = make_delphi_stream(
                    audioData.data(), (uint32_t)audioData.size());
...
static bool load_frame_png(
    const fs::path& pngPath, uint32_t& width, uint32_t& height, std::vector<uint8_t>& out)
{
    ...
    stbi_uc* rgba = stbi_load_from_memory(
        (const stbi_uc*)bytes.data(), (int)bytes.size(), &w, &h, &comp, 4);
...
static void* load_sprite_obj(const std::string& name, const fs::path& spriteDir)
{
    ...
    if (!sp || sp == (void*)sp_vmt)
    {
        gm_log("Sprite %s: ctor FAILED", name.c_str());
        return nullptr;
    }
        ...
        [&](auto& k, auto& v)
    {
        ...
        {
            gm_log("  WARN: sprite field parse error for %s key=%s", name.c_str(),
            k.c_str());
        }
    });
...
static bool gm80_action_fill_in(void* act, uint32_t libId, uint32_t actId)
{
    ...
    for (uint32_t li = 0; li < libCnt; li++)
    {
        ...
        for (uint32_t ai = 0; ai < acnt; ai++)
        {
            ...
            for (int i = 0; i < 8; i++)
                set_obj_u32(
                    act, 36 + i * 4, *(uint32_t*)(tpl + 88 + i * 4)); // param_types
            return true;
        }
    }
    return false;
...
static bool fill_in_safe(void* act, uint32_t libId, uint32_t actId)
{
    ...
    __except (EXCEPTION_EXECUTE_HANDLER)
    {
        gm_log("fill_in_safe: AV code=0x%X lib=%u id=%u act=0x%X", GetExceptionCode(),
            libId, actId, (uint32_t)act);
        return false;
    }
}
...
{
    std::vector<std::string> sprites, sounds, bgs, paths, scripts, objects, rooms, fonts,
        timelines;
...
static void parse_actions_into_event(void* ev, const std::string& body,
    const std::vector<std::string>& objectNames, const std::string& owner)
{
            ...
            [&](auto& k, auto& v)
        {
            ...
            else if (k == "applies_to")
            {
                ...
                if (v == "other") set_obj_i32(act, 68, -2);
                    ...
                    set_obj_i32(act, 68, -1);
                else if (v.empty())
                    set_obj_i32(act, 68, -4);
                else
                    set_obj_i32(act, 68,
                    resolve_name_warn(
                    owner.c_str(), "applies_to", v, objectNames));
            }
            else if (k == "invert")
                set_obj_bool(act, 108, v == "1");
            else if (k == "repeats")
            {
                hasRepeats = true;
                pset[0] = true;
        ...
        for (int j = 0; j < 8; j++)
        {
            ...
            if (ptype >= 5 && ptype <= 14 && ptype != 13)
            {
                ...
                if (!pv.empty())
                {
                    ...
                    {
                    case 5:
                        idx = AssetNameMaps::find(g_action_names.sprites, pv);
                        break;
                    case 6:
                        idx = AssetNameMaps::find(g_action_names.sounds, pv);
                        break;
                    case 7:
                        idx = AssetNameMaps::find(g_action_names.bgs, pv);
                        break;
                    case 8:
                        idx = AssetNameMaps::find(g_action_names.paths, pv);
                        break;
                    case 9:
                        idx = AssetNameMaps::find(g_action_names.scripts, pv);
                        break;
                    case 10:
                        idx = AssetNameMaps::find(g_action_names.objects, pv);
                        break;
                    case 11:
                        idx = AssetNameMaps::find(g_action_names.rooms, pv);
                        break;
                    case 12:
                        idx = AssetNameMaps::find(g_action_names.fonts, pv);
                        break;
                    case 14:
                        idx = AssetNameMaps::find(g_action_names.timelines, pv);
                        break;
                    }
                    if (idx < 0)
...
static int parse_event_header(const std::string& line, int& evIndex)
{
    ...
    // trim \r
    while (!evName.empty() && (evName.back() == '\r' || evName.back() == ' '))
        evName.pop_back();
...
static void obj_add_event(void* obj, int evType, int evIndex, void* ev)
{
    ...
    *(uint32_t*)((uint8_t*)mem + 4) = newLen;
    ...
    for (uint32_t i = len; i < newLen; i++)
        dst[i] = (i == (uint32_t)evIndex) ? ev : make_event();
    ...
    const std::vector<std::string>& spriteNames)
{
    ...
    fs::path txtPath = objDir /
        (name + ".txt"); // flat layout (gm82save: set_extension("txt"))
    std::string txt = read_file(txtPath);
        ...
        [&](auto& k, auto& v)
    {
        if (k == "sprite")
            set_obj_i32(
            obj, 4, resolve_name_warn(name.c_str(), "sprite", v, spriteNames));
        else if (k == "visible")
            set_obj_bool(obj, 9, v == "1");
        else if (k == "solid")
            ...
            set_obj_i32(
            obj, 20, resolve_name_warn(name.c_str(), "parent", v, objectNames));
        else if (k == "mask")
            set_obj_i32(
            obj, 24, resolve_name_warn(name.c_str(), "mask", v, spriteNames));
    });
...
static void load_room_instances(void* rm, const fs::path& subDir,
    const std::vector<std::string>& objectNames, const std::vector<std::string>& bgNames);
    ...
    const std::vector<std::string>& objectNames, const std::vector<std::string>& bgNames)
{
    ...
    for (int i = 0; i < 8; i++)
    {
            ...
            [&](auto& k, auto& v)
        {
            if (k == p0) set_obj_bool(rm, vo, v == "1");
                ...
                resolve_name_warn(
                name.c_str(), "view follow target", v, objectNames));
        });
    }
        ...
        [&](auto& k, auto& v)
    {
        if (k == "remember") set_obj_bool(rm, 768, v == "1");
            ...
            set_obj_bool(rm, 781, v == "1");
        else if (k == "show_tiles")
            set_obj_bool(rm, 782, v == "1");
        else if (k == "show_backgrounds")
            set_obj_bool(rm, 783, v == "1");
        else if (k == "show_foregrounds")
            set_obj_bool(rm, 784, v == "1");
        else if (k == "show_views")
            set_obj_bool(rm, 785, v == "1");
        else if (k == "delete_underlying_objects")
            set_obj_bool(rm, 786, v == "1");
                    ...
                    *(uint32_t*)((uint8_t*)mem + 4) = (uint32_t)rows.size();
                    ...
                    for (size_t i = 0; i < rows.size(); i++)
                    {
                        ...
                        *(int32_t*)(inst + 8) = rows[i].obj;
                        ...
                        int32_t iid = (int32_t)(++(
                                *(uint32_t*)(glob_base() + ADDR_LAST_INSTANCE_ID)));
                        *(int32_t*)(inst + 12) = iid;
                        if (gm80_valid_code_hash(rows[i].hash))
                            gm80_instance_hash_set(iid, rows[i].hash);
...
static bool load_assets_simple(
    const char* dirName, LoadFn loader, const ResInfo* ri, const fs::path& root)
{
    ...
    for (uint32_t i = 0; i < cnt; i++)
    {
        ...
        if (obj)
        {
            ...
            if (isSprites)
            {
                fnThumb = (uint32_t)base + 0xF959C;
                extraArr = 0x1E9118;
            }
            else if (isBg)
            {
                fnThumb = (uint32_t)base + 0x121060;
                extraArr = 0x1E90A4;
            }
            if (fnThumb)
            {
...
static bool load_assets_ctx(const char* dirName, LoadFnCtx loader, const ResInfo* ri,
    const fs::path& root, const std::vector<std::string>& ctxA,
...
bool gm80_load_project(void* gm_base, const std::wstring& wpath)
{
    ...
    gm_log("Loading objects...");
    {
        ...
        load_assets_ctx(
            "objects", load_object, find_res("objects"), root, names, sprites);
    }
    gm_log("Objects done.");
    ...
    // 1899/12/30 in the project properties. Use GM's own Now() (sub_40CF18).
    {
        uint8_t* b = (uint8_t*)g_load_base;
        uint32_t fn = (uint32_t)b +
            0xCF18; // sub_40CF18 = Now() (GetLocalTime→EncodeDate+EncodeTime)
        double now = 0.0;
        __asm {
            call fn
    ...
    // set it to the metadata file path, not the folder.
    {
        ...
        int alen = WideCharToMultiByte(
            CP_ACP, 0, wproj.c_str(), (int)wproj.size(), NULL, 0, NULL, NULL);
        if (alen > 0)
        {
            std::string aproj(alen, '\0');
            WideCharToMultiByte(
                CP_ACP, 0, wproj.c_str(), (int)wproj.size(), &aproj[0], alen, NULL, NULL);
            write_glob_str(0x1EA27C, aproj);
            gm_log("gm80_load_project: GM80_ProjectPath set to '%s'", aproj.c_str());
        }
    }
