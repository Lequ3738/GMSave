...
static std::string read_delphi_str(void** ppStr)
{
    ...
    uint32_t len = hdr[1];
    if (len > 4096 || refcnt < -10 || refcnt > 1000000) return "(invalid str header)";
...
static int __stdcall check_and_do_gm80_save()
{
    ...
    // doesn't silently write into the metadata file treated as a directory.
    {
            ...
            pn.size() > 5 && pn.compare(pn.size() - 5, 5, L".gm80") == 0)
        {
            ...
            gm_log("Save: normalized save path to folder '%S'", g_gm80_save_path.c_str());
        }
    }
...
static void read_name_array(
    uint8_t* base, uint32_t name_off, uint32_t cnt_off, std::vector<std::string>& out)
{
...
static void __stdcall do_gm80_save_if_needed()
{
    ...
    if (ok && gm80_diag_any()) gm80_diag_show("Game Maker 8.0");
    ...
    if (!ok)
    {
        ...
        if (!err.empty())
        {
            ...
            MessageBoxA(
                gm80_prompt_owner(), msg.c_str(), "Game Maker 8.0", MB_OK | MB_ICONERROR);
        }
    }
}
...
static void __stdcall log_gmk_save_ctx()
{
    ...
    gm_log("GMK save ctx: cwd='%s' projPath='%s'", cwd,
        (pp && *pp && !IsBadStringPtrA(*pp, 1024)) ? *pp : "(null/bad)");
    if (pp && *pp && !IsBadStringPtrA(*pp, 1024))
    {
        ...
        {
            *slash = '\0';
            if (SetCurrentDirectoryA(dir)) gm_log("GMK save ctx: cwd set to '%s'", dir);
        }
        ...
        {
            gm_log("GMK save ctx: target attrs=0x%X readonly=%d", attrs,
                (attrs & FILE_ATTRIBUTE_READONLY) ? 1 : 0);
        }
    }
}
...
static void __stdcall do_gmk_save_direct()
{
    ...
    {
        gm_log("GMK direct save: Inner exception code=0x%X", GetExceptionCode());
        gm80_progress_close();
        return;
    }
    if (!ok)
...
static void __stdcall msg_log(void* callerAddr)
{
    ...
    gm_log("ShowMessage called: text='%s' callerRVA=0x%X (callerAddr=0x%p)", buf,
        callerRva, callerAddr);
}
...
static int __stdcall check_and_do_gm80_load()
{
    ...
    if (len < 6)
    {
        gm_log("check_gm80: len=%u path='%s' -> 0", (uint32_t)len, projPath);
        return 0;
    }
    bool is_gm80 = (_stricmp(projPath + len - 5, ".gm80") == 0);
    if (!is_gm80)
    {
        gm_log("check_gm80: not .gm80 path='%s' -> 0", projPath);
        return 0;
    }
...
static const char k_gm80_filter[] = "GameMaker 8.0 files (*.gmk)\0*.gmk\0"
...
static void hook_comdlg32_iat(HMODULE gm_base)
{
    ...
    DWORD impRva = nt->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT]
        .VirtualAddress;
    if (!impRva) return;
    IMAGE_IMPORT_DESCRIPTOR* imp = (IMAGE_IMPORT_DESCRIPTOR*)((uint8_t*)gm_base + impRva);
    for (; imp->Name; imp++)
    {
        const char* dll = (const char*)((uint8_t*)gm_base + imp->Name);
        if (_stricmp(dll, "comdlg32.dll") != 0) continue;
        IMAGE_THUNK_DATA* thunk = (IMAGE_THUNK_DATA*)((uint8_t*)gm_base +
            imp->FirstThunk);
        IMAGE_THUNK_DATA* orig = (IMAGE_THUNK_DATA*)((uint8_t*)gm_base +
            imp->OriginalFirstThunk);
        for (; thunk->u1.AddressOfData; thunk++, orig++)
        {
            if (IMAGE_SNAP_BY_ORDINAL(orig->u1.Ordinal)) continue;
            IMAGE_IMPORT_BY_NAME* ibn = (IMAGE_IMPORT_BY_NAME*)((uint8_t*)gm_base +
                orig->u1.AddressOfData);
            if (strcmp(ibn->Name, "GetSaveFileNameA") == 0 && !g_real_GetSaveFileNameA)
            {
                g_real_GetSaveFileNameA = (BOOL(WINAPI*)(
                    LPOPENFILENAMEA))thunk->u1.Function;
                DWORD oldp;
            ...
            else if (strcmp(ibn->Name, "GetOpenFileNameA") == 0 &&
                !g_real_GetOpenFileNameA)
            {
                g_real_GetOpenFileNameA = (BOOL(WINAPI*)(
                    LPOPENFILENAMEA))thunk->u1.Function;
                DWORD oldp;
                if (VirtualProtect(&thunk->u1.Function, 4, PAGE_READWRITE, &oldp))
                {
...
bool ide_hooks_install(HMODULE gm_base)
{
    ...
    gm_log("CompareText hooks installed at %u sites",
        (uint32_t)(sizeof(cmptext_sites) / sizeof(cmptext_sites[0])));
    ...
    // (ExtractFilePath of the embedded path) resolves to the project root.
    {
        ...
        g_trampoline_5D453C = VirtualAlloc(
